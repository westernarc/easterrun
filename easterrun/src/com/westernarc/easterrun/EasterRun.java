...
public class EasterRun implements ApplicationListener {
	...
	//Boots speed
	final float constGroundMaxRateWithBoots = constGroundMaxRate * 1.3f;
	...
	public void create() {		
		...
		if(Math.random() > 0.8) {
			mdlGroundStraight = loadModel("models/ground2");
			mdlGroundCornerR = loadModel("models/corner2");
			mdlGroundCornerL = loadModel("models/corner2L");
			mdlGroundBridge = loadModel("models/bridge2");
			mdlGroundOutcrop = loadModel("models/outcrop2");
		} else {
	...
	public void update(float tpf) {
		...
		for(int i = 0; i < groundMax; i++) {
			...
			if(actGround[i].position.x > groundLength * 1.3f) {
